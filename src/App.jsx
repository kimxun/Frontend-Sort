...
function App() {
    ...
    for (let i = 0; i < n - 1; i++) {
      for (let j = i + 1; j < n; j++) {
        ...
        if (arrayCopy[i] > arrayCopy[j]) {
          ...
          stepsList.push({
            step: stepCounter++,
            description: `Hoán đổi a[${i}] và a[${j}] → [${arrayCopy.join(", ")}]`,
            array: [...arrayCopy],
            comparing: [],
            swapping: [i, j],
            sorted: [],
  ...
  const applyStep = (step) => {
    ...
    setHistory(prev => {
      const exists = prev.some(h => h.step === step.step);
  ...
  const initializeSteps = (newArray) => {
    const newSteps = generateInterchangeSteps(newArray);
    setSteps(newSteps);
    setStepIndex(0);
    if (newSteps.length > 0) {
      const firstStep = newSteps[0];
      setArray(firstStep.array);
      setComparing(firstStep.comparing || []);
      setSwapping(firstStep.swapping || []);
      setSorted(firstStep.sorted || []);
      setCurrentLine(firstStep.currentLine || 0);
      setVariables(firstStep.variables || []);
      setHistory([{
        step: firstStep.step,
        description: firstStep.description,
        array: firstStep.array,
      }]);
    }
  };
  ...
  const handleStart = () => {
    if (stepIndex < steps.length - 1) setIsRunning(true);
  };
  const handlePause = () => setIsRunning(false);
  const handleReset = () => {
    setIsRunning(false);
    const defaultArray = [45, 20, 80, 65, 30, 95, 10, 55];
  ...
  return (
    <div className="app-container dark">
      <SortingLayout
        ...
        canStepForward={!isRunning && stepIndex < steps.length - 1}
        array={array}
        comparing={comparing}
        swapping={swapping}
        sorted={sorted}
